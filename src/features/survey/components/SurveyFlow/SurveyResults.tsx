...
import { buildResultsShareUrl } from '../../../result/services/resultsShare'
...
export function SurveyResults({
  ...
  useEffect(() => {
    if (!resultsActionMessage) {
      return
    }

    const timer = window.setTimeout(() => setResultsActionMessage(null), 2500)
  ...
  const trainerRegionAnswer = selectedRegion?.label.replace(/^\d+\.\s*/, '') ?? 'Not set yet'
  ...
  const starterFinalEvolution = useStarterFinalEvolutionResult(selectedStarter?.pokemonNumber ?? null)
  ...
  const normalizedRandomTeammateId = Number.isNaN(randomTeammatePokemonId) ? null : randomTeammatePokemonId
  ...
  const teamGridItems = [
    ...
    deterministicRegionalTeammateResult.isLoading
      ...
      : deterministicRegionalTeammateResult.errorMessage
        ...
        : deterministicRegionalTeammateResult.pokemonNumber && deterministicRegionalTeammateResult.pokemonName
          ? {
              id: 'regional',
    ...
    deterministicTypeTeammateResult.isLoading
      ...
      : deterministicTypeTeammateResult.errorMessage
        ...
        : deterministicTypeTeammateResult.pokemonNumber && deterministicTypeTeammateResult.pokemonName
          ? {
              id: 'type',
    ...
    deterministicTeamMemberFiveResult.isLoading
      ...
      : deterministicTeamMemberFiveResult.errorMessage
        ...
        : deterministicTeamMemberFiveResult.pokemonNumber && deterministicTeamMemberFiveResult.pokemonName
          ? {
              id: 'deterministic',
  ...
  const handleCopyResultsLink = async () => {
    try {
      if (navigator.clipboard?.writeText) {
        ...
        textArea.value = resultsShareUrl
      ...
      setResultsActionMessage('Copied to clipboard')
    } catch {
      setResultsActionMessage('Unable to copy link right now')
    }
  }

  return (
    <div className={`game-panel results-panel ${resultsPresentationPhase === 'complete' ? '' : 'results-panel-staged'}`}>
        ...
        <div className="result-reveal-overlay" role="dialog" aria-modal="true">
          <div className="result-reveal-modal">
            ...
            <div className="result-reveal-actions">
              <button type="button" className="pixel-button" onClick={handleAcknowledgeReveal}>
                YAY! 🙌
              </button>
              <button type="button" className="pixel-button" onClick={handleAcknowledgeReveal}>
                Nice!🙂
        ...
        <>
            ...
            <div className="result-info-grid">
              {infoGridItems.map((resultItem) => (
                <div key={resultItem.id} className="result-info-card">
                  ...
                  {resultItem.value ? <strong className="result-info-value">{resultItem.value}</strong> : null}
                  {resultItem.mediaSrc ? (
                    <img
          ...
          {resultsActionMessage ? (
            <div className="results-toast">
              <p className="saved-note">{resultsActionMessage}</p>
            </div>
          ) : null}
        </>
      ) : null}
